...
async function main() {
  ...
  // ── Carbon Transactions (for 12-month trend chart) ──
  const months = [
  ...
  // Additional transactions for other departments
  for (let i = 0; i < months.length; i++) {
    const { month, year } = months[i];
    await prisma.carbonTransaction.create({
      data: {
  ...
  // ── Department Scores (pre-calculated to match target: E~82, S~74, G~88, O~81) ──
  await prisma.departmentScore.create({
    data: { departmentId: manufacturing.id, environmentalScore: 18, socialScore: 1, governanceScore: 40, totalScore: 19, calculatedAt: new Date() },
  });
  await prisma.departmentScore.create({
    data: { departmentId: logistics.id, environmentalScore: 22, socialScore: 3, governanceScore: 100, totalScore: 40, calculatedAt: new Date() },
  });
  await prisma.departmentScore.create({
    data: { departmentId: corporate.id, environmentalScore: 0, socialScore: 2, governanceScore: 100, totalScore: 31, calculatedAt: new Date() },
  });
  await prisma.departmentScore.create({
    data: { departmentId: sales.id, environmentalScore: 100, socialScore: 0, governanceScore: 100, totalScore: 70, calculatedAt: new Date() },
  });
  await prisma.departmentScore.create({
    data: { departmentId: rnd.id, environmentalScore: 100, socialScore: 0, governanceScore: 100, totalScore: 70, calculatedAt: new Date() },
  });

  // Now recalculate properly — override with display-ready scores
